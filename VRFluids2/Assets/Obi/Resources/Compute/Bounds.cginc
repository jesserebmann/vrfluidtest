...
struct aabb
{
    ...
    void FromTriangle(float4 v1, float4 v2, float4 v3, float margin)
    {
        min_ = min(min(v1, v2), v3) - float4(margin, margin, margin, 0);
        max_ = max(max(v1, v2), v3) + float4(margin, margin, margin, 0);
    }

    void FromEdge(float4 v1, float4 v2, float radius)
    {
