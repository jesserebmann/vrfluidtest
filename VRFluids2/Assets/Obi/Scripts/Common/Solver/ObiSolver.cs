...
namespace Obi
{
    ...
    public sealed class ObiSolver : MonoBehaviour
    {
        ...
        [Tooltip("Velocity range that increases global foam potential.")]
        public Vector2 velocityRange = new Vector2(2, 4);

        [Tooltip("Vorticity range that increases global foam potential")]
        public Vector2 vorticityRange = new Vector2(2, 3);

        [Tooltip("Rate at which global foam potential increases.")]
        public float foamGenerationRate = 200;

        [Tooltip("Duration of 100% global foam potential")]
        [Range(0, 1)]
        public float foamDuration = 0.8f;
        ...
        public void StartSimulation(float stepDelta, int simulationSteps)
        {
            if (simulationSteps > 0)
            {
                ...
                if (initialized && maxStepsPerFrame > 0)
                {
                    ...
                    // Update colliders/rigidbodies after adding new actors to make sure collision materials are up to date.
                    // Also call it before SimulationStart, so that constraints referencing colliders (such as Pin constraints in attachments)
                    // use handle data that's up to date. 
                    using (m_UpdateColliderWorld.Auto())
                    {
                        ObiColliderWorld.GetInstance().FlushHandleBuffers();
                        ObiColliderWorld.GetInstance().UpdateWorld(simulatedTime);
