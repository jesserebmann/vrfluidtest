...
namespace Obi
{
    ...
    public class DynamicRenderBatch<T> : IRenderBatch where T : IMeshDataProvider, IActorRenderer
    {
        ...
        {
            // if the dynamic data is not available (such as when the batch is intended for GPU use), return right away.
            /*if (dynamicVertexData == null || !dynamicVertexData.isCreated)
                return;*/
