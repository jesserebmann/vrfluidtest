    ...
    public class ComputeSolverImpl : ISolverImpl
    {
        ...
        public IObiJobHandle Substep(IObiJobHandle handle, float stepTime, float substepTime, int steps, float timeLeft)
        {
            ...
            if (activeParticleCount > 0)
            {
                ...
                // Predict positions:
                solverShader.SetVector("velocityRange", m_Solver.velocityRange);
                solverShader.SetVector("vorticityRange", m_Solver.vorticityRange);
                solverShader.SetFloat("foamGenerationRate", m_Solver.foamGenerationRate);
                solverShader.SetFloat("foamPotentialMultiplier", Mathf.Pow(1 - Mathf.Clamp(m_Solver.foamDuration, 0, 1), substepTime));
        ...
        private void UpdateDiffuseParticles(float deltaTime)
        {
            ...
            if (system != null && m_Solver.maxFoamParticles > 0)
            {
                ...
                foamShader.SetBuffer(sortDataKernel, "sortedVelocities", particleGrid.sortedUserDataVelColor);
                ...
                for (int i = 0; i < system.renderers.Count; ++i)
                {
                    ...
                    if (system.renderers[i].actor.solverIndices.computeBuffer != null &&
                        system.renderers[i].emitPotential.computeBuffer != null)
                    {
                        ...
                        foamShader.SetBuffer(emitFoamKernel, "emitPotentials", system.renderers[i].emitPotential.computeBuffer);
                ...
                foamShader.SetBuffer(updateFoamKernel, "positions", particleGrid.sortedPrevPosOrientations);
                foamShader.SetBuffer(updateFoamKernel, "velocities", particleGrid.sortedUserDataVelColor);
                foamShader.SetBuffer(updateFoamKernel, "fluidSimplices", particleGrid.sortedSimplexToFluid);
                foamShader.SetBuffer(updateFoamKernel, "principalRadii", principalRadiiBuffer);
                foamShader.SetBuffer(updateFoamKernel, "sortedSimplexIndices", particleGrid.sortedSimplexIndices);
