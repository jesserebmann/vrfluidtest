...
namespace Obi
{
    ...
    { 
        ...
        public ComputeFoamRenderSystem(ObiSolver solver) : base (solver)
        {
            ...
            renderBatch = new ProceduralRenderBatch<DiffuseParticleVertex>(0, Resources.Load<Material>("ObiMaterials/Common/FoamParticles"), new RenderBatchParams(true));
        ...
        public override void Setup()
        {
            using (m_SetupRenderMarker.Auto())
            {
                for (int i = 0; i < renderers.Count; ++i)
                {
                    ...
                    if (renderers[i].emitPotential != null && renderers[i].emitPotential.computeBuffer == null)
                        renderers[i].emitPotential.AsComputeBuffer<float>();
                }
            }
        }
