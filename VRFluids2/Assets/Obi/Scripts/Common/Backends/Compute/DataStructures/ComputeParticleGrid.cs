...
namespace Obi
{
    public class ComputeParticleGrid : IDisposable
    {
        ...
        public GraphicsBuffer sortedFluidData;              // Used by Density constraints: no need to be sorted (constraints output). Reuse for velocities (foam advection and mesher).

        public GraphicsBuffer sortedPositions;              // Used by Density constraints: needs to be sorted once per iteration.
        public GraphicsBuffer sortedPrevPosOrientations;    // Used by Density constraints: needs to be sorted once per iteration. Reuse for orientations (mesher).
        public GraphicsBuffer sortedUserDataVelColor;       // Used by Density constraints: needs to be sorted once per iteration. Reuse for velocities (foam advection) and colors (mesher). TODO: not reusing for velocitoes.
        ...
        private void Clear()
        {
            ...
            if (sortedUserDataVelColor != null)
                sortedUserDataVelColor.Dispose();
        ...
        public bool SetCapacity(int capacity, uint maxParticleContacts, uint maxParticleNeighbors)
        {
                ...
                maxParticleContacts != this.maxParticleContacts)
            {
                ...
                sortedUserDataVelColor = new GraphicsBuffer(GraphicsBuffer.Target.Structured, capacity, 16);
