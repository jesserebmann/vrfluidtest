...
namespace Obi
{
    ...
    struct PredictPositionsJob : IJobParallelFor
    {
        ...
        [ReadOnly] public float2 velocityRange;
        [ReadOnly] public float2 vorticityRange;
        [ReadOnly] public float foamGenerationRate;
        [ReadOnly] public float foamPotentialMultiplier;
        ...
        public void Execute(int index)
        {
            ...
            if (inverseRotationalMasses[i] > 0)
            {
                ...
                // update foam generation potential, store it in angular velocity 4th component:
                float vorticityPotential = math.remap(vorticityRange.x, vorticityRange.y,0,1,math.length(angularVel.xyz));
                float velocityPotential = math.remap(velocityRange.x, velocityRange.y,0,1,math.length(velocities[i].xyz));
                float trappedAir = vorticityPotential * velocityPotential * foamGenerationRate * deltaTime;
                angularVelocities[i] = new float4(angularVel, math.saturate(angularVelocities[i].w * foamPotentialMultiplier + trappedAir));
            }
