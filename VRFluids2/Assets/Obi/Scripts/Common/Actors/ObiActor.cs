        ...
        public virtual void Interpolate(float simulatedTime, float substepTime)
        {
            ...
            if (!Application.isPlaying && isLoaded)
            {
                ...
                for (int i = 0; i < solverIndices.count; i++)
                {
                    ...
                    {
                        m_Solver.positions[k] = m_Solver.startPositions[k] = m_Solver.endPositions[k] = l2sTransform.MultiplyPoint3x4(sourceBlueprint.positions[i]);
                    }

                    if (sourceBlueprint.orientations != null && i < sourceBlueprint.orientations.Length)
                    {
                        m_Solver.orientations[k] = m_Solver.startOrientations[k] = m_Solver.endOrientations[k] = l2sRotation * sourceBlueprint.orientations[i];
                    }
                }
            }
